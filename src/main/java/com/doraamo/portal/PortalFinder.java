...
public final class PortalFinder {
    ...
    @Nullable
    public static NearestPortal findNearest(World world, EntityPlayer player) {
        if (world == null || player == null) {
            return null;
        }
        ChunkPos origin = new ChunkPos(player.getPosition());
        ...
        for (int cx = -SEARCH_CHUNK_RADIUS; cx <= SEARCH_CHUNK_RADIUS; cx++) {
            for (int cz = -SEARCH_CHUNK_RADIUS; cz <= SEARCH_CHUNK_RADIUS; cz++) {
                if (!world.isChunkGeneratedAt(origin.x + cx, origin.z + cz)) {
                    continue;
                }
                Chunk chunk = world.getChunkFromChunkCoords(origin.x + cx, origin.z + cz);
                for (TileEntity te : chunk.getTileEntityMap().values()) {
                    if (!(te instanceof TileEntityPortalDoor)) {
                        continue;
                    }
                    TileEntityPortalDoor portal = (TileEntityPortalDoor) te;
                    BlockPos p = portal.getPos();
                    double dist = player.getDistanceSq(p.getX() + 0.5D, p.getY() + 0.5D, p.getZ() + 0.5D);
                    if (dist < bestDist) {
                        bestDist = dist;
    ...
    /** Compass octant 0=S … clockwise, matching Minecraft yaw. */
    public static String directionKey(EntityPlayer player, BlockPos target) {
        ...
        switch (sector) {
            case 0:
                return "doraamo.tuner.dir.s";
            case 1:
                return "doraamo.tuner.dir.sw";
            case 2:
                return "doraamo.tuner.dir.w";
            case 3:
                return "doraamo.tuner.dir.nw";
            case 4:
                return "doraamo.tuner.dir.n";
            case 5:
                return "doraamo.tuner.dir.ne";
            case 6:
                return "doraamo.tuner.dir.e";
            case 7:
            default:
                return "doraamo.tuner.dir.se";
        }
    }
}
